...
star stars[1000];
int score = 0, star_count;
float star_angle = 0, star_offset = 1.5;
...
GLvoid drawStar(GLMmodel* model, GLuint mode);
GLvoid DrawGLScene();
...
void SetupWorld();
void read_doors();
void append_doors(int ref, int start, float xleft, float zleft);
void remove_doors(int i);
...
//	GLfloat lightpos[] = {-50, 6, -70, 1.0};
	...
	else if(view_mode==4)	// Tile View
	{
		...
		gluLookAt(sector1.triangle[2*tile_index].vertex[0].x + 2,
				sector1.triangle[2*tile_index].vertex[0].y + 2 ,
				sector1.triangle[2*tile_index].vertex[0].z + 3,
	...
	glutPrint(0.200f, 0.155f, glutFonts[3], score_print, 1.0f, 0.0f, 0.0f, 0.0f);
	time_left = 300 - (int)elapsed;
	if((int)time_left % 60 < 10)
		sprintf(score_print, "Time - 0%d:0%d", (int)time_left/60, (int)time_left % 60);
	else
		sprintf(score_print, "Time - 0%d:%d", (int)time_left/60, (int)time_left % 60);
	glutPrint(0.200f, 0.135f, glutFonts[3], score_print, 1.0f, 0.0f, 0.0f, 0.0f);
	...
	for(i=0; i<5; i++)
	{
		...
		glutPrint(0.200f, 0.115f - 0.02* i, glutFonts[3], pos_map[i], 0.0f, 1.0f, 0.0f, 0.0f);
...
void update(int value)
{
	...
	//printf("%d %d\n", (current_id-2)/5, (current_id-2)%5 );
	if(current_id == 29 && score >= 10)
	{
		strcpy(sound, "teleport.mp3");
		play(sound);
		xtrans += (sector1.triangle[2*current_id].vertex[0].x - sector1.triangle[2*current_id + 10].vertex[0].x);
		current_id +=5;
	}
	if(current_id == 34 && score >= 15)
	{
		strcpy(sound, "teleport.mp3");
		play(sound);
		xtrans += (sector1.triangle[2*current_id].vertex[0].x - sector1.triangle[2*current_id + 10].vertex[0].x);
...
void read_doors()
{
	float x, y, z, u, v;
	int vert,localtex;
	FILE *filein = fopen("Data/doors.txt", "rt");
	char oneline[255];
	...
	sector2.triangle = (TRIANGLE *) malloc(sizeof(TRIANGLE)*1000);
	...
	for (loop = 0; loop < (int)numtriangles; loop++)
	{
		...
		for (vert = 0; vert < 3; vert++) {
			...
			sector2.triangle[loop].tex=localtex;
			...
			sector2.triangle[loop].id = loop/2;
...
void SetupWorld()
{
	...
	door_begin_id = sector1.numtriangles;
	// Make the doors
	for(i=0; i<n-1; i++)
	{
		for(j= (i%2); j<(i%2)+ m-1; j++)
		{
			ref = 6;
			// Door 4
			start = sector1.numtriangles;			
			...
			append_doors(ref, start, xleft, zleft);

			ref = 4;
			start = sector1.numtriangles;
			xleft = 1.0 * tile_width * (i+1) - tile_width / 2.0 + (i+1) * tile_dist + tile_height/2;
			zleft = 1.0 * tile_height * j - tile_height / 2.0 + j * tile_dist + tile_width/2;
	...
	for(i=0; i<n; i++)
	{
		...
		ref = 4;
		start = sector1.numtriangles;
		xleft = 1.0 * tile_width * j - tile_width / 2.0 + j * tile_dist + tile_height/2;
		zleft = 1.0 * tile_height * i - tile_height / 2.0 + i * tile_dist + tile_width/2;
		append_doors(ref, start, xleft, zleft);
	...
	for(i=0; i<n; i++)
	{
		for(j=0; j<m; j++)
		{
			...
			stars[k].x = xleft - tile_height/4;
			stars[k].z = zleft - tile_width/4;
			stars[k++].y = 0.1;
...
void remove_doors(int i)
{
	...
	for(j=0; j<m-1; j++)
	{
		start = door_begin_id + 16 * i + 4 * j;


		sector1.triangle[start].vertex[2].x+=1.5;
		sector1.triangle[start].vertex[2].z-=1;

		sector1.triangle[start+1].vertex[0].x+=1.5;
		sector1.triangle[start+1].vertex[0].z-=1;

		sector1.triangle[start+1].vertex[2].x+=1.5;
		sector1.triangle[start+1].vertex[2].z-=1;

		sector1.triangle[start].vertex[0].x+=0.5;
		sector1.triangle[start].vertex[1].x+=0.5;
		sector1.triangle[start+1].vertex[1].x+=0.5;
...
void stars_check()
{
	...
	for(i=0; i<star_count; i++)
	{
		if(fabs(-xtrans -stars[i].x ) < 0.1 && fabs(-ztrans - stars[i].z) < 0.1)
		{
			stars[i].x = -100000;
			score++;
			if(i== star_count - 5)
			...
			{
				remove_doors(0);
				strcpy(sound,"coin.wav");
				play(sound);
				strcpy(sound, "creakingdoor.wav");
				play(sound);
			}
			else if(score == 10)
			{
...
void append_doors(int ref, int start, float xleft, float zleft)
{
	...
	{
		for (vert = 0; vert < 3; vert++) {
			sector1.triangle[start + loop].tex = sector2.triangle[ref + loop].tex;
			sector1.triangle[start + loop].vertex[vert].x = sector2.triangle[ref + loop].vertex[vert].x + xleft;
			sector1.triangle[start + loop].vertex[vert].y = sector2.triangle[ref + loop].vertex[vert].y;
			sector1.triangle[start + loop].vertex[vert].z = sector2.triangle[ref + loop].vertex[vert].z + zleft;
			sector1.triangle[start + loop].vertex[vert].u = sector2.triangle[ref + loop].vertex[vert].u;
			sector1.triangle[start + loop].vertex[vert].v = sector2.triangle[ref + loop].vertex[vert].v;
			sector1.triangle[start + loop].id = (loop+ start)/2;
...
GLvoid DrawGLScene()
{
	...
	for (loop=0; loop<(int)numtriangles; loop++) {        // loop through all the triangles
		...
		if(sector1.triangle[loop].tex==5)
		{
			...
			if(disc_color < 50)
				disc_offset = disc_offset * -1;
			if(sector1.triangle[loop].id == 29 && score >= 1)
				glColor3f(0, 0, disc_color/ 255.0);
			else if(sector1.triangle[loop].id == 34 && score >= 1)
				glColor3f(0, 0, disc_color/ 255.0);
			else if(sector1.triangle[loop].id == 39 && score >= 1)
				glColor3f(0, 0, disc_color/ 255.0);
			else
...
void play(char a[])
{
	...
	if(pid==0)
	{
		...
		if(APPLE)
		{
			sprintf(command, "afplay %s", a);
			system(command);
		}
		else
		{
			sprintf(command, "cvlc %s", a);
			system(command);
		}
		exit(0);
	}
