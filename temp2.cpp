...
int tile_index = 2;
float  tile_width=3, tile_height=3, tile_dist=4;
float connecting_dist = 0, connecting_offset = 0.03;
...
void drawScene() 
{
	...
	if(view_mode==0)	// First Person Mode
		...
		gluLookAt(-xcenter, 5 , -zcenter, 0, z_look_helicopter/2 + z_helicopter/10.0, x_look_helicopter/2 + x_helicopter/10.0, 0, 1, 0);
...
void update(int value) 
{
	...
	if(connecting_dist > tile_dist || connecting_dist < -1.0)
	{
		connecting_offset*=-1;
	}

	if(connecting_dist > 0 && connecting_dist < tile_dist-1)
	{
		create_connecting(102, 203, 0, connecting_dist, 1);
		create_connecting(183, 365, 1, connecting_dist, 1);
	}
	
	if(!jump_flag)
	{
		if(current_id >= 102 && current_id < 183)
		{
			if(connecting_dist > 0 && connecting_dist < tile_dist - 1)
				ztrans -= connecting_offset;
		}
		if(current_id >= 183 && current_id < 264)
		{
			if(connecting_dist > 0 && connecting_dist < tile_dist - 1)
	...
	if(fall_flag==1)
	{
		...
		if(fall_dist < 0.3)
		{
			...
			if(current_id ==1) // TODO : HANDLE JUMP IN WATER
			{
...
//			y = rand1/200.0;
	...
	create_connecting(id, indx, 0, 0, 0);
	create_connecting(183, 365, 1, 0, 0);
...
float terrain_collision()
{
	...
	for(i=0; i<sector1.numtriangles; i++)
	{
		...
		for (vert = 0; vert < 3; vert++) 
		{
			if(sector1.triangle[i].id == current_id)
				continue;
//			if( fabs(sector1.triangle[i].vertex[vert].y - ytrans )< 0.01)
//				continue;
...
GLvoid drawSteve(GLMmodel* model, GLuint mode)
{
	...
	{
		if(strcmp(model ->pathname , boxed_obj[j].name)==0)
			break;
	}
	boxed_obj[j].boxes.clear();
	group = model->groups;
	box b;
	//printf("ads%s\n", group->name);
	while (group) 
...
void create_connecting(int id, int indx, int flag, float offset, int flag2)
{
	...
	for(i=0; i<9; i++)
	{
		for(j=0; j<9; j++)
		{
			...
			if(flag==0)
			{
				xleft = xleft + tile_width/2;
				zleft = zleft + tile_height + offset;
			}
			else if(flag==1)
			{
				xleft = xleft + tile_width + offset;
				zleft = zleft + tile_height/2;
			}

			indx++;

			sector1.triangle[indx].tex = 3;
			sector1.triangle[indx].id = id;
...
GLvoid DrawGLScene()
{
	...
	for (loop=0; loop<(int)numtriangles; loop++) {        // loop through all the triangles
		...
		{
		glEnable(GL_TEXTURE_2D); // Enable texture mapping.
		glBindTexture(GL_TEXTURE_2D, texture4[filter]); // 2d texture (x and y size)
		}
		else if(sector1.triangle[loop].tex==7)
		{
		glEnable(GL_TEXTURE_2D); // Enable texture mapping.
		glBindTexture(GL_TEXTURE_2D, texture5[filter]); // 2d texture (x and y size)
		}
		else
		{
